...
mod Collateral {
    ...
    struct Storage {
        ...
        shuttle_id: u32,
        /// The total balance of LP Tokens this vault currently owns
        total_balance: u256,
    ...
    impl CollateralImpl of ICollateral<ContractState> {
        ...
        fn exchange_rate(self: @ContractState) -> u256 {
            ...
            if supply == 0 {
                ...
                // Exchange rate is Assets/Supply
                let balance = self.total_balance.read();
                balance.div_wad(supply)
            }
        }
        ...
        fn flash_redeem(
            ...
            let total_balance = self.total_balance.read();
            let shares = assets.full_mul_div_up(total_supply, total_balance);

            /// 2. Transfer LP to redeemer
            self.underlying.read().transfer(redeemer, assets);
